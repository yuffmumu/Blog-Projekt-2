...
### <a name="9"></a>Respawn:
Unser nächstes Ziel war es. dass wenn das L_Teil den Boden, bzw. in unserem Fall den bottom_detector berüht soll ein neues L_Teil gespawnt werden. Bis jetzt wurde es nur zu Anfang im world constructor erzeugt. Also erstellten wir den boolean detect, der immer dann true wurde, wenn das L_Teil den bottom_detector berührt. Dann wollten wir, wenn detect true wird ein neues L_Teil spawnen. Wir versuchten dies mithilfe einer if-schleife. Wenn die Bedingung detect == true ist, dann führe die Methode addObject(new L_Teil, 8, 0) aus. Es kam jedoch der Fehler "cannot find symbol - method addObject(L_Teil, int, int)" raus. Wir schlossen also daraus, dass man nur in der World class Objekte erzeugen kann. Also kopierten wir unsere if-Schleife einfach in MyWorld. Doch wieder kam eine Fehlermeldung ("cannot find symbol - variable detect"). Das heißt die Variable detect war nicht übergreifend. Von Snap waren wir es gewöhnt, dass alle Objekte standartmäßig auf alle Variablen zugreifen konnten. Wir haben wirklich lange im Internet recherchiert und einiges ausprobiert. (vllt versch. Lös. Ansätze erläutern) Schließlich sind wir auf eine Seite gestoßen, die Objekte innnerhalb einer Actor Klasse erzeugen konnte, indem man sich Methoden aus andern Klassen holt. Dies funktioniert mit der sogenannten Punkt notation. Wir brauchten eine Methode der World Klasse, also schrieben wir getWorld() vor die addObject Methode. Als wir es dann ausprobierten funktionierte alles. Das heißt wir brauchten die Variable detect, und die if-Schleife in MyWorld nicht mehr, da wir ja direkt in der Actor Klasse neue Objekte erzeugen konnten. 
